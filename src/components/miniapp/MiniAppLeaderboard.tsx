...
export const MiniAppLeaderboard: React.FC = () => {
  ...
  const sorted = [...users].sort((a, b) => b.tasksCompleted - a.tasksCompleted);
    ...
    <div className="space-y-5 animate-slide-up">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-400">{currentRank > 0 ? `Votre rang: #${currentRank}` : 'Complétez des tâches pour apparaître'}</p>
        </div>
      </div>
        ...
        {sorted.map((user, i) => {
          ...
          return (
            <div key={user.id} className={`glass-card-light p-3.5 flex items-center gap-3 ${isMe ? 'border border-blue-500/30 bg-blue-500/5' : ''}`}>
              ...
              <p className="text-sm font-semibold text-emerald-400 flex-shrink-0">{user.tasksCompleted} tâches</p>
            </div>
          );
        })}
      </div>
