...
async def cmd_start(msg: types.Message):
    ...
    await msg.answer(
        f"🚀 <b>Welcome to TonCipher!</b>\n\n"
        f"TonCipher is a Telegram Mini App that lets you earn real <b>GRAM</b> on the TON Blockchain.\n\n"
        f"💎 <b>Complete tasks</b>\n"
        f"👥 <b>Invite friends</b>\n"
        f"🎁 <b>Redeem promo codes</b>\n"
        f"💸 <b>Withdraw directly to your TON wallet</b>\n\n"
        f"Join the TonCipher community and start earning today!\n\n"
        f"👇 <b>Open the app below and begin your journey now.</b>",
        parse_mode="HTML",
        reply_markup=kb,
)
...
async def handle_wd_callback(cb: types.CallbackQuery) -> None:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if action == "ok":
            ...
            try:
                async with db.execute(
                    "SELECT first_name, username FROM users WHERE telegram_id=?", (uid,)
                ) as cur:
...
async def api_check_membership(request: web.Request) -> web.Response:
    ...
    if BOT_TOKEN:
        if not init_data or not _validate_init_data(init_data, BOT_TOKEN):
            return web.json_response({"error": "Authentication required"}, status=401, headers=_CORS)
        if telegram_id and _init_data_user_id(init_data) != telegram_id:
...
async def api_admin_approve_withdrawal(request: web.Request) -> web.Response:
    ...
    if tx_row:
        ...
        try:
            ...
            date_str_wp = processed_dt.strftime("%a, %d %b %Y · %H:%M UTC")
        except Exception:
            from datetime import datetime as _dt2
        ...
        if tx_link and tx_hash:
            ...
            hash_display_wp = f'<a href="{tx_link}">{short_hash}</a>'
        else:
            hash_display_wp = "Manual"
        await _notify_channel(
            await _configured_withdrawal_channel(),
            f"✅ <b>Withdrawal Successful</b>\n\n"
...
async def api_proof_image(request: web.Request) -> web.Response:
    ...
    # Accept initData from header only — the frontend uses JS fetch so the
    # credential never appears in the URL (logs, referrer headers, browser history).
    init_data = request.headers.get("X-Init-Data", "")
    if BOT_TOKEN:
        if not init_data or not _validate_init_data(init_data, BOT_TOKEN):
