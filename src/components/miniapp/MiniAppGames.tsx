...
const CrashLineGame: React.FC<{ onBack: () => void; streak: number; onResult: OnResult }> = ({ onBack, onResult }) => {
  ...
  if (phase === 'waiting') {
    if (queuedBet !== null) {
      btnLabel = `Cancel (${queuedBet.toFixed(2)} GRAM)`; btnBg = '#334155'; btnColor = '#94a3b8';
      btnFn = () => { setQueuedBet(null); queuedBetR.current = null; };
    } else {
      btnLabel = `BET ${bet.toFixed(2)} GRAM`; btnBg = 'linear-gradient(135deg,#3b82f6,#6366f1)'; btnColor = '#fff';
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 60, background: '#080c1a', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 9, fontWeight: 800, color: '#fff', background: '#E50539', padding: '2px 6px', borderRadius: 4, letterSpacing: '0.05em' }}>LIVE</span>
        </div>
        ...
        <div style={{ padding: '10px 12px 10px', borderBottom: '1px solid #1e2847', background: 'linear-gradient(180deg, #0a0e1e 0%, #080c1a 100%)', position: 'sticky', top: 0, zIndex: 2 }}>
          ...
          {queuedBet !== null && (
            <div style={{ textAlign: 'center', fontSize: 10, color: '#475569', marginTop: 5, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
              ...
              Queued: <TonLogo size={9} />{queuedBet.toFixed(2)} GRAM ·{' '}
              <span onClick={() => { setQueuedBet(null); queuedBetR.current = null; }} style={{ color: '#3b82f6', cursor: 'pointer', fontWeight: 700 }}>Cancel</span>
            </div>
          )}
        </div>
